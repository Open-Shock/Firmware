...
size_t webSocketSendFrame(AsyncClient* client, bool final, uint8_t opcode, bool mask, uint8_t* data, size_t len)
{
  ...
  if (len) {
    if (len && mask) {
      size_t i;
      for (i = 0; i < len; i++) data[i] = data[i] ^ mbuf[i % 4];
    }
...
{
  if (_status != WS_CONNECTED) return;
  if (code) {
    uint8_t packetLen = 2;
    if (message != NULL) {
...
{
  if (_status == WS_CONNECTED) _queueControl(new AsyncWebSocketControl(WS_PING, data, len));
}
...
void AsyncWebSocketClient::_onData(void* pbuf, size_t plen)
{
  ...
  while (plen > 0) {
    if (!_pstate) {
      ...
      if (plen < 2) goto _exceptionHandleFailPartialHeader;
      _pinfo.index  = 0;
      ...
      if (_pinfo.len == 126) {
        ...
        if (plen < 8) goto _exceptionHandleFailPartialHeader;

        _pinfo.len
          = headerBuf[9] | (uint16_t)(headerBuf[8]) << 8 | (uint32_t)(headerBuf[7]) << 16 | (uint32_t)(headerBuf[6]) << 24 | (uint64_t)(headerBuf[5]) << 32 | (uint64_t)(headerBuf[4]) << 40 | (uint64_t)(headerBuf[3]) << 48 | (uint64_t)(headerBuf[2]) << 56;
        dataPayloadOffset += 8;
        plen -= 8;
      }

      if (_pinfo.masked) {
        if (plen < 4) goto _exceptionHandleFailPartialHeader;
...
_headerParsingSuccessful:
    ...
    if (_pinfo.masked) {
      for (size_t i = 0; i < datalen; i++) data[i] ^= _pinfo.mask[(_pinfo.index + i) % 4];
    }
    ...
    if ((datalen + _pinfo.index) < _pinfo.len) {
      ...
      if (_pinfo.opcode == WS_DISCONNECT) {
        ...
        if (datalen != AWSC_PING_PAYLOAD_LEN || memcmp(AWSC_PING_PAYLOAD, data, AWSC_PING_PAYLOAD_LEN) != 0) _server->_handleEvent(this, WS_EVT_PONG, NULL, data, datalen);
    ...
    if (datalen > 0) data[datalen] = datalast;
...
{
  _clients.remove_first([=](AsyncWebSocketClient* c) { return c->id() == client->id(); });
}
...
{
  return _clients.count_if([](AsyncWebSocketClient* c) { return c->status() == WS_CONNECTED; });
}
...
void AsyncWebSocket::close(uint32_t id, uint16_t code, const char* message)
{
  ...
  if (c) c->close(code, message);
}

void AsyncWebSocket::closeAll(uint16_t code, const char* message)
{
  for (const auto& c : _clients) {
    if (c->status() == WS_CONNECTED) c->close(code, message);
  }
}
