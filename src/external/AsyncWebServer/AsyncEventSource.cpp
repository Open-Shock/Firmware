...
*/
#include "external/AsyncWebServer/AsyncEventSource.h"
#include "Arduino.h"
  ...
  if (message != NULL) {
    ...
    do {
      ...
      if (nextN == NULL && nextR == NULL) {
        ...
        lineStart = nextLine;
        if (lineStart == ((char*)message + messageLen)) ev += "\r\n";
      }
  ...
  : _messageQueue(LinkedList<AsyncEventSourceMessage*>([](AsyncEventSourceMessage* m) { delete m; }))
{
  ...
  _server = server;
  _lastId = 0;
  if (request->hasHeader("Last-Event-ID")) _lastId = atoi(request->getHeader("Last-Event-ID")->value().c_str());
...
void AsyncEventSourceClient::_onAck(size_t len, uint32_t time)
{
  while (len && !_messageQueue.isEmpty()) {
    ...
    if (_messageQueue.front()->finished()) _messageQueue.remove(_messageQueue.front());
  }
...
void AsyncEventSource::_addClient(AsyncEventSourceClient* client)
{
  ...
  _clients.add(client);
  if (_connectcb) _connectcb(client);
}
...
{
  return _clients.count_if([](AsyncEventSourceClient* c) { return c->connected(); });
}
